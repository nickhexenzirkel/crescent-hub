...
interface DebrisPoly { cx: number; cy: number; verts: [number, number][]; rotSpeed: number; rot: number; driftX: number; driftY: number; alpha: number; size: number; }
interface ShootingStar { x: number; y: number; vx: number; vy: number; life: number; maxLife: number; angle: number; }
interface GalaxyStar { dist: number; angle: number; r: number; alpha: number; bright: boolean; }
interface Particle { x: number; y: number; vx: number; vy: number; life: number; maxLife: number; r: number; purple: boolean; }
interface SurfacePatch { x: number; y: number; rx: number; ry: number; a: number; dark: boolean; }

// Destroyed planet shard
interface Shard {
  ox: number; oy: number;   // orbit offset from planet center (normalized by planet r)
  orbitR: number; orbitSpeed: number; orbitPhase: number;
  verts: [number, number][];
  rot: number; rotSpeed: number;
  size: number; alpha: number;
}
interface BrokenPlanet {
  x: number; y: number;   // 0-1 screen fraction
  r: number;              // pixel radius fraction of min(w,h)
  shards: Shard[];
  crackSeeds: { a1: number; a2: number; }[];
  rotSpeed: number;
}

function makeSurface(n: number): SurfacePatch[] {
...
function makeBrokenPlanet(x: number, y: number, r: number): BrokenPlanet {
  ...
  const shards: Shard[] = Array.from({ length: shardCount }, (_, i) => {
    ...
    const dist = rng(1.3, 5.0); // orbit distance from planet center (in units of r)
    return {
      ...
      size: rng(0.06, 0.22), // fraction of r
      alpha: rng(0.4, 0.85),
    };
  });
...
const DEBRIS: DebrisPoly[] = Array.from({ length: 40 }, () => ({
...
function makeGalaxyStars(count: number, arms: number): GalaxyStar[] {
...
// Broken planets — lower-left cluster + top-right corner
const BROKEN_PLANETS: BrokenPlanet[] = [
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // ── nebula blobs ─────────────────────────────────────────────────────────
    const blob = (bx: number, by: number, br: number, c0: string, a0: number, c1: string, a1: number) => {
      const w = W(), h = H();
    ...
    // ── shooting stars — rare, slow, toward upper-right ──────────────────────
    const spawnShooter = () => {
    ...
    // ── orbiting galaxies ────────────────────────────────────────────────────
    const drawGalaxy = (stars: GalaxyStar[], cx: number, cy: number, radius: number, spin: number, tilt: number) => {
      ctx.save();
    ...
    const drawGalaxies = () => {
      ...
      // G3: small, orbits left-center area, tighter loop
    ...
    // ── planet with rotating surface ─────────────────────────────────────────
    const drawPlanet = (cx: number, cy: number, r: number, opts: {
      light: string; dark: string;
      glow: string; glowR: number; glowA: number;
      rotSpeed: number; rotOffset: number;
      surface: SurfacePatch[];
      surfaceLight: string; surfaceDark: string;
      rings?: { color: string; alpha: number; tilt: number; angle: number; };
    }) => {
    ...
    // ── bloom ────────────────────────────────────────────────────────────────
    const drawBloom = (cx: number, cy: number, r: number) => {
    ...
    const drawBrokenPlanets = () => {
      ...
      BROKEN_PLANETS.forEach(bp => {
        ...
        // Planet core (cracked, but still a sphere)
        ...
        // Rotating surface patches on core
        ctx.save();
    ...
    const frame = () => {
      ...
      // Broken planets — draw before main planets so they appear layered behind
      drawBrokenPlanets();

      // Planet 1 — upper-left diagonal, rings +35°
      ...
      // Planet 2 — center-right diagonal, rings -25°
