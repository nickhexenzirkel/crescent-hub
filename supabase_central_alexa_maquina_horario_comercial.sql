-- Máquina do Tempo — nova regra de horário (substitui o corte simples de "< 18h" de
-- supabase_central_alexa_maquina_horario.sql): só conta stream/play de SEGUNDA A SEXTA,
-- das 08:00 às 12:00 E das 13:00 às 17:00 (horário de Brasília). Fora disso (antes das
-- 8h, no almoço 12h-13h, depois das 17h, ou em sábado/domingo) NÃO conta.
-- Brasil não observa mais horário de verão desde 2019, então 'America/Sao_Paulo' é
-- UTC-3 fixo o ano todo (sem risco de DST bagunçar o corte).
-- Rode no SQL Editor do Supabase, depois de supabase_central_alexa_maquina.sql,
-- supabase_central_alexa_maquina_horario.sql e supabase_central_alexa_maquina_periodo.sql.

-- Helper: true se o instante cai dentro do "horário comercial" contado pela Máquina do
-- Tempo. Centraliza a regra num lugar só (evita repetir a mesma condição em 6 lugares
-- e trocar tudo de novo se a regra mudar de novo no futuro).
create or replace function public.maquina_horario_valido(ts timestamptz)
returns boolean
language sql immutable as $$
  select
    extract(dow from ts at time zone 'America/Sao_Paulo') between 1 and 5  -- seg(1)..sex(5); dom=0, sáb=6
    and (
      extract(hour from ts at time zone 'America/Sao_Paulo') between 8 and 11   -- 08:00–11:59
      or extract(hour from ts at time zone 'America/Sao_Paulo') between 13 and 16 -- 13:00–16:59
    );
$$;
