...
const TrophyIcon = () => (
  <svg width="26" height="28" viewBox="0 0 26 28" fill="none">
    <path d="M7 2h12v9c0 3.87-2.69 7.1-6.34 7.9L12 19l-.66-.1C7.69 18.1 5 14.87 5 11V2z" fill="white"/>
    <path d="M7 4.5H3.5A1.5 1.5 0 002 6v1.5A3.5 3.5 0 005 11" stroke="white" strokeWidth="1.5" strokeLinecap="round" fill="none"/>
    <path d="M19 4.5h3.5A1.5 1.5 0 0124 6v1.5A3.5 3.5 0 0121 11" stroke="white" strokeWidth="1.5" strokeLinecap="round" fill="none"/>
    <rect x="10" y="19" width="6" height="4" rx="1" fill="white"/>
    <rect x="7" y="23" width="12" height="3" rx="1.5" fill="white"/>
    <circle cx="13" cy="10" r="2.2" fill="rgba(255,230,0,0.75)"/>
  </svg>
);

const RainbowFlagIcon = () => (
  <svg width="28" height="20" viewBox="0 0 28 20" fill="none">
    <defs><clipPath id="fc"><rect width="28" height="20" rx="3"/></clipPath></defs>
    <g clipPath="url(#fc)">
      <rect y="0"     width="28" height="3.34" fill="#FF0018"/>
      <rect y="3.34"  width="28" height="3.33" fill="#FF8C00"/>
      <rect y="6.67"  width="28" height="3.33" fill="#FFED00"/>
      <rect y="10"    width="28" height="3.33" fill="#008026"/>
      <rect y="13.33" width="28" height="3.33" fill="#004DFF"/>
      <rect y="16.66" width="28" height="3.34" fill="#750787"/>
    </g>
  </svg>
);

const FloatingThemeBtn = ({ themeKey, onConfetti }) => {
  const [hov, setHov] = useState(false);
  const isBrasil  = themeKey === 'vozBrasil'  || themeKey === 'vozBrasilDark';
  const isOrgulho = themeKey === 'orgulho'    || themeKey === 'orgulhoDark';
  if (!isBrasil && !isOrgulho) return null;

  const bg = isBrasil
    ? 'linear-gradient(140deg,#009C3B 0%,#00A040 40%,#FFE000 100%)'
    : 'linear-gradient(140deg,#FF0018 0%,#FF8C00 20%,#FFED00 40%,#008026 60%,#004DFF 80%,#750787 100%)';
  ...
  return (
    <button
      ...
      title={isBrasil ? 'Voz do Brasil 🏆' : 'Mês do Orgulho 🏳️‍🌈'}
      style={{
        ...
        background:bg,
        boxShadow: hov
          ? '0 8px 28px rgba(0,0,0,0.35), 0 2px 8px rgba(0,0,0,0.18)'
          : '0 4px 14px rgba(0,0,0,0.22)',
        transform: hov ? 'scale(1.15)' : 'scale(1)',
        transition:'all .2s cubic-bezier(.16,1,.3,1)',
        animation: isOrgulho && !hov ? 'themeBtnPulse 3s ease-in-out infinite' : 'none',
        outline:'none',
      }}>
      {isBrasil ? <TrophyIcon/> : <RainbowFlagIcon/>}
    </button>
  );
};

const CONFETTI_PALETTES = {
  vozBrasil:     ['#FFE000','#F5C800','#FFDD00','#00A040','#009C3B','#003DA5','#0040CC','#FFFFFF'],
  vozBrasilDark: ['#FFE000','#F5C800','#FFDD00','#00A040','#009C3B','#003DA5','#0040CC','#FFFFFF'],
...
export { LavaLamp, Moon, StarDivider, Logo, BrandLogo, UnikoIcon, Card, Tag, Btn, Inp, SHead, AvatarCircle, Confetti, FloatingThemeBtn };
