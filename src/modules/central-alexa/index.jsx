...
const CentralAlexa = ({onBack, userPhoto, initialTab}) => {
  ...
  /* `sobreEscuro` é usado quando a fila é aberta DENTRO da tela cheia do
     player: lá o fundo é a capa do álbum borrada e escura, e as cores do tema
     claro (cinzas de texto secundário, azul do `gold`, vermelho do perigo)
     ficavam ilegíveis em cima dela — "2 músicas", "A seguir", "4 votos =
     skip automático" e o nome de quem pediu praticamente sumiam.
  ...
  const renderQueueCard = (opts = {}) => {
    const C = opts.sobreEscuro ? {
      text:   '#FFFFFF',
      textT:  'rgba(255,255,255,.74)',
      textD:  'rgba(255,255,255,.60)',
      border: 'rgba(255,255,255,.16)',
      gold:     T.goldV || T.goldL || T.gold,   // a variante clara do tema, legível no escuro
      goldLine: T.goldV || T.goldL || T.gold,
      goldGl: 'rgba(255,255,255,.10)',
      sh:     'none',
      perigo:   '#FF8A9B',
      perigoGl: 'rgba(255,138,155,0.14)',
      perigoLn: 'rgba(255,138,155,0.40)',
    } : { ...T, perigo:'#C04050', perigoGl:'rgba(192,64,80,0.08)', perigoLn:'rgba(192,64,80,0.3)' };
    return (
              ...
              <div style={{borderRadius:16,background:opts.sobreEscuro?'rgba(14,14,22,.88)':opts.noBlur?'rgba(255,255,255,.05)':cardBg,
                    ...
                    {!isAdmin && (() => {
                      ...
                      return (
                        <span style={{
                          ...
                          color: remaining===0 ? C.perigo : remaining===1 ? "#E08030" : C.gold,
                          background: remaining===0 ? "C.perigoGl" : remaining===1 ? "rgba(224,128,48,0.08)" : C.goldGl,
                          border: `1px solid ${remaining===0 ? "C.perigoLn" : remaining===1 ? "rgba(224,128,48,0.3)" : C.goldLine+"44"}`,
                    ...
                    {isAdmin && queue.length > 0 && (
                      <button
                        ...
                        style={{display:"flex",alignItems:"center",gap:4,padding:"3px 8px",borderRadius:6,
                          ...
                          color:C.perigo,cursor:"pointer",fontSize:10,fontWeight:700,outline:"none",
                ...
                {festLoading
                  ? <div style={{padding:"32px",textAlign:"center",color:C.textT,fontSize:13}}>
                      <div style={{width:20,height:20,borderRadius:"50%",border:`2px solid ${C.gold}`,borderTopColor:"transparent",animation:"spin 0.7s linear infinite",margin:"0 auto 8px"}}/>
                      Carregando fila...
                    </div>
                  : queue.length===0
                    ? <div style={{padding:"32px",textAlign:"center",color:C.textT,fontSize:13}}>
                        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke={C.textT} strokeWidth="1.5" strokeLinecap="round" style={{margin:"0 auto 8px",display:"block"}}><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>
                    ...
                    : (() => {
                          ...
                          <div key={s.id} className="ca-row" style={{display:"flex",alignItems:"center",flexWrap:isMobile?"wrap":"nowrap",gap:isMobile?8:12,padding:isMobile?"10px 12px":"11px 16px",borderTop:idx===0?"none":`1px solid ${C.border}`,background:iAmPlaying?C.goldGl:"transparent",transition:"background .15s"}}>
                            ...
                            <div style={{width:22,textAlign:"center",flexShrink:0}}>
                              {iAmPlaying
                                ? <div style={{display:"flex",alignItems:"flex-end",gap:1,height:14,justifyContent:"center"}}>
                                    {[1,2,3].map(j=><div key={j} style={{width:2,borderRadius:1,background:C.gold,animation:`alexaEq${j} ${0.4+j*0.1}s ease-in-out infinite alternate`,minHeight:3}}/>)}
                                  </div>
                                : <span style={{fontSize:11,color:C.textD}}>{idx+1}</span>
                              }
                            </div>
                            {/* Capa */}
                            {s.album_art
                              ? <img src={s.album_art} alt="" style={{width:iAmPlaying?44:36,height:iAmPlaying?44:36,borderRadius:iAmPlaying?9:7,objectFit:"cover",flexShrink:0,boxShadow:iAmPlaying?`0 4px 16px ${C.goldLine}44`:"none",transition:"all .2s"}}/>
                              : <div style={{width:iAmPlaying?44:36,height:iAmPlaying?44:36,borderRadius:iAmPlaying?9:7,background:C.goldGl,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>🎵</div>
                            }
                            {/* Título + artista */}
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{fontSize:iAmPlaying?14:13,fontWeight:iAmPlaying?700:500,color:iAmPlaying?C.gold:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.title}</div>
                              <div style={{fontSize:11,color:C.textT,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.artist}</div>
                            </div>
                            {/* Pedido por */}
                            {(()=>{
                            ...
                            {iAmPlaying && isAdmin && (
                              <button onClick={()=>handleVote(s)} title="Pular (Admin)"
                                style={{display:"flex",alignItems:"center",gap:4,padding:"3px 9px",borderRadius:6,border:`1.5px solid ${C.gold}55`,background:C.goldGl,color:C.gold,cursor:"pointer",fontSize:11,fontWeight:700,outline:"none",transition:"all .15s"}}>
                            ...
                            {canReorder && (
                              <div style={{display:"flex",flexDirection:"column",gap:2,flexShrink:0}}>
                                <button onClick={()=>moveSong(s,-1)} disabled={isFirst||!!movingId}
                                  ...
                                  style={{display:"flex",alignItems:"center",justifyContent:"center",width:22,height:13,borderRadius:5,border:`1px solid ${C.border}`,background:"transparent",color:C.textD,cursor:(isFirst||movingId)?"default":"pointer",outline:"none",padding:0,opacity:(isFirst||movingId)?0.25:0.7,transition:"opacity .15s"}}
                                  onMouseEnter={e=>{ if(!isFirst&&!movingId){e.currentTarget.style.opacity="1";e.currentTarget.style.borderColor=`${C.gold}66`;e.currentTarget.style.color=C.gold;} }}
                                  onMouseLeave={e=>{ if(!isFirst&&!movingId){e.currentTarget.style.opacity="0.7";e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.textD;} }}>
                                ...
                                <button onClick={()=>moveSong(s,1)} disabled={isLast||!!movingId}
                                  ...
                                  style={{display:"flex",alignItems:"center",justifyContent:"center",width:22,height:13,borderRadius:5,border:`1px solid ${C.border}`,background:"transparent",color:C.textD,cursor:(isLast||movingId)?"default":"pointer",outline:"none",padding:0,opacity:(isLast||movingId)?0.25:0.7,transition:"opacity .15s"}}
                                  onMouseEnter={e=>{ if(!isLast&&!movingId){e.currentTarget.style.opacity="1";e.currentTarget.style.borderColor=`${C.gold}66`;e.currentTarget.style.color=C.gold;} }}
                                  onMouseLeave={e=>{ if(!isLast&&!movingId){e.currentTarget.style.opacity="0.7";e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.textD;} }}>
                            ...
                            {canDelete && (
                              <button onClick={()=>openReplace(s)}
                                ...
                                style={{display:"flex",alignItems:"center",justifyContent:"center",width:26,height:26,borderRadius:6,border:`1.5px solid ${C.border}`,background:"transparent",color:C.textD,cursor:"pointer",outline:"none",flexShrink:0,opacity:0.7,transition:"opacity .15s"}}
                                onMouseEnter={e=>{e.currentTarget.style.opacity="1";e.currentTarget.style.borderColor=`${C.gold}66`;e.currentTarget.style.color=C.gold;}}
                                onMouseLeave={e=>{e.currentTarget.style.opacity="0.7";e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.textD;}}>
                            ...
                            {canDelete && (
                              <button onClick={async()=>{ await api('delete',`/api/queue/${s.id}`); loadQueue(); }}
                                ...
                                style={{display:"flex",alignItems:"center",justifyContent:"center",width:26,height:26,borderRadius:6,border:`1.5px solid ${C.border}`,background:"transparent",color:C.textD,cursor:"pointer",outline:"none",flexShrink:0,opacity:0.7,transition:"opacity .15s"}}
                                onMouseEnter={e=>{e.currentTarget.style.opacity="1";e.currentTarget.style.borderColor="rgba(192,64,80,0.4)";e.currentTarget.style.color=C.perigo;}}
                                onMouseLeave={e=>{e.currentTarget.style.opacity="0.7";e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.textD;}}>
                            ...
                            <span style={{fontSize:10,color:C.textD,minWidth:28,textAlign:"right"}}>{s.duration_str||"—"}</span>
                          </div>
                        );
                      };
                      ...
                      return (
                        <>
                          ...
                          {playingSong && (
                            <>
                              <div style={{padding:"8px 16px 6px",display:"flex",alignItems:"center",gap:6,borderBottom:`1px solid ${C.border}`}}>
                                <svg width="10" height="10" viewBox="0 0 24 24" fill={C.gold} stroke="none"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                                <span style={{fontSize:10,fontWeight:700,color:C.gold,textTransform:"uppercase",letterSpacing:".08em"}}>Tocando Agora</span>
                              </div>
                          ...
                          {pending.length > 0 && (
                            <>
                              <div style={{padding:"8px 16px 6px",display:"flex",alignItems:"center",justifyContent:"space-between",borderTop:`1px solid ${C.border}`,borderBottom:`1px solid ${C.border}`,background:isDark?"rgba(255,255,255,0.02)":"rgba(0,0,0,0.02)"}}>
                                <div style={{display:"flex",alignItems:"center",gap:6}}>
                                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={C.textD} strokeWidth="2" strokeLinecap="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>
                                  <span style={{fontSize:10,fontWeight:700,color:C.textD,textTransform:"uppercase",letterSpacing:".08em"}}>A Seguir — {pending.length} {pending.length===1?"música":"músicas"}</span>
                                </div>
                                <span style={{fontSize:10,color:C.textT}}>{VETO} votos = skip automático</span>
                              </div>
  ...
  return (
    <div className={`ca-root${festBgVideo ? ' ca-bgvid-on' : ''}`} style={{minHeight:"100vh",background:"transparent",fontFamily:"var(--font-body)",position:"relative",overflowX:"hidden"}}>
      ...
      {isMobile && cur && !nowPlayingOpen && (
        <div onClick={() => { setNowPlayingOpen(true); setTelaPlayer("player"); }} role="button" aria-label="Abrir tela do que está tocando"
        ...
        <div style={{ position:"fixed", inset:0, zIndex:2000, background: isDark ? "#0b0b12" : "#181022", display:"flex", flexDirection:"column", overflow:"hidden" }}>
          ...
          <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", height:"100%", padding:"0 22px", paddingTop:"max(20px, env(safe-area-inset-top, 20px))", paddingBottom:"calc(20px + env(safe-area-inset-bottom, 0px))" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"6px 0 12px" }}>
              ...
              <span style={{ fontSize:16, fontWeight:800, color:"#fff", letterSpacing:".08em", textTransform:"uppercase", fontFamily:"var(--font-brand)" }}>
            ...
            {telaPlayer === "fila" ? (
              <div style={{ flex:1, minHeight:0, overflowY:"auto" }}>
                {renderQueueCard({ noBlur:true, sobreEscuro:true })}
              </div>
            ) : telaPlayer === "letra" ? (
