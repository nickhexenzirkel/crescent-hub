...
// UNIKO FIT — estilo GymRats: transforma treino em competição saudável entre
...
// treinos e ranking por frequência. Módulo próprio (ao lado da Prisma Store no
// seletor de módulos), liberado pra TODOS. Sem servidor próprio: tudo via
// Supabase (tabelas uniko_fit_* — rodar supabase_uniko_fit.sql) + bucket de
// fotos `uniko-fit-fotos`.
...
@keyframes fitFade { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: none; } }
...
.fit-fade { animation: fitFade .3s ease both; }
.fit-float { animation: fitFloat 2.4s ease-in-out infinite; }
.fit-btn { transition: transform .12s, filter .12s; }
.fit-btn:hover:not(:disabled) { transform: translateY(-1px); filter: brightness(1.06); }
...
.fit-scroll { scrollbar-width: thin; scrollbar-color: ${ENERGIA}99 rgba(128,128,128,.14); }
.fit-scroll::-webkit-scrollbar { width: 8px; }
...
`;
  ...
  const toggleReacao = async (checkinId, emoji) => {
    ...
    try {
      if (jaEssa) await supabase.from('uniko_fit_reactions').delete().eq('checkin_id', checkinId).eq('player', name);
      else await supabase.from('uniko_fit_reactions').upsert({ checkin_id: checkinId, player: name, emoji }, { onConflict: 'checkin_id,player' });
  ...
  const postarCheckin = async () => {
    ...
    try {
      ...
      const { error: upErr } = await supabase.storage.from('uniko-fit-fotos').upload(path, ckFile, { contentType: ckFile.type || undefined, upsert: false });
      ...
      const { error } = await supabase.from('uniko_fit_checkins').insert({ player: name, photo_url: pub.publicUrl, caption: ckCaption.trim() || null });
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase.from('uniko_fit_chat').select('*').order('created_at', { ascending: false }).limit(80);
  ...
  const enviarChat = async () => {
    ...
    setChatMsg('');
    try { await supabase.from('uniko_fit_chat').insert({ player: name, texto }); }
  ...
  const loadRanking = useCallback(async () => {
    ...
    for (;;) {
      const { data, error } = await supabase.from('uniko_fit_checkins').select('player,created_at').range(from, from + PAGE - 1);
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'transparent', fontFamily: 'var(--font-body)', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      {/* ── Topbar ── */}
      <div style={{ height: 56, background: T.topbarBg || cardBg, backdropFilter: 'blur(28px)', WebkitBackdropFilter: 'blur(28px)', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', padding: '0 18px', gap: 12, position: 'sticky', top: 0, zIndex: 200, boxShadow: `0 1px 20px ${ENERGIA}22` }}>
        ...
        {!isMobile && <span style={{ fontSize: 14, fontWeight: 700, color: T.text, fontFamily: 'var(--font-brand)', letterSpacing: '.04em' }}>Uniko Fit</span>}
        <div style={{ flex: 1 }} />
        {/* Perfil do usuário */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '4px 10px 4px 4px', borderRadius: 999, background: `${ENERGIA}18`, border: `1px solid ${ENERGIA}44` }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '16px' : '24px 28px 40px' }}>
        <div style={{ maxWidth: 980, margin: '0 auto', fontFamily: 'var(--font-body)' }}>
          ...
          <div style={{ borderRadius: 16, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16,
            ...
            <div className="fit-float" style={{ width: 58, height: 58, borderRadius: 16, flexShrink: 0, background: '#fff',
            ...
            <div style={{ flex: 1, minWidth: 0, position: 'relative' }}>
              <div style={{ fontFamily: 'var(--font-brand)', fontSize: 22, fontWeight: 800, color: '#fff' }}>Uniko Fit 💪</div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
            {SUBNAV.map(s => {
              ...
              return (
                <button key={s.id} className="fit-btn" onClick={() => setSubTab(s.id)}
            ...
            <div style={{ maxWidth: 460, margin: '0 auto' }}>
              {feed === null ? (
                ...
                <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                  ...
                  <button className="fit-btn" onClick={() => setSubTab('checkin')}
                ...
                <div className="fit-feed" style={{ height: 'min(74vh, 780px)', overflowY: 'auto', borderRadius: 18, border: `1px solid ${T.border}`, boxShadow: T.sh }}>
                  {feed.map(post => {
                    const r = reacoes[post.id] || { counts: {}, mine: null };
                    const totalReacoes = Object.values(r.counts).reduce((a, b) => a + b, 0);
                    return (
                      <div key={post.id} className="fit-card" style={{ position: 'relative', height: '100%', minHeight: '100%', background: '#111' }}>
          ...
          {/* ── CHECK-IN (postar treino) ── */}
          {subTab === 'checkin' && (
            <div className="fit-fade" style={{ maxWidth: 460, margin: '0 auto', background: cardBg, border: `1px solid ${T.border}`, borderRadius: 16, padding: 22, boxShadow: T.sh }}>
              ...
              {ckPreview ? (
                ...
                <button className="fit-btn" onClick={() => ckFileRef.current?.click()}
              ...
              <button className="fit-btn" onClick={postarCheckin} disabled={ckSaving || !ckFile}
          ...
          {/* ── CHAT DO GRUPO ── */}
          {subTab === 'chat' && (
            <div className="fit-fade" style={{ maxWidth: 620, margin: '0 auto', background: cardBg, border: `1px solid ${T.border}`, borderRadius: 16,
              ...
              <div className="fit-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 9 }}>
                {chat === null ? (
              ...
              <div style={{ display: 'flex', gap: 8, padding: 12, borderTop: `1px solid ${T.border}` }}>
                ...
                <button className="fit-btn" onClick={enviarChat} disabled={!chatMsg.trim()}
          ...
          {/* ── RANKING ── */}
          {subTab === 'ranking' && (
            <div className="fit-fade" style={{ maxWidth: 560, margin: '0 auto' }}>
