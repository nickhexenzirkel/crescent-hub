-- "Capture o Uniko" — permite ATÉ 3 capturas por evento (antes era só 1: quem
-- capturava primeiro "trancava" o evento pra todo mundo; depois foi ajustado
-- pra 5, agora pra 3). As 3 primeiras pessoas que conseguirem capturar ganham;
-- a 4ª tentativa em diante encontra o evento já esgotado. Rode no SQL Editor
-- do Supabase (seguro rodar de novo mesmo se já rodou a versão com 5).

-- Cada capturador ocupa um "slot" (1 a 3) dentro do mesmo evento.
...
-- composta (event_id, player) — permite até 3 jogadores DIFERENTES por
...
-- No máximo 1 pessoa por slot, por evento — é isso que trava em 3 no total.
...
-- Função atômica: tenta ocupar o PRÓXIMO slot livre (1 a 3) pra esse
...
-- evita a corrida de "4 pessoas capturando ao mesmo tempo" sem precisar de
-- lock explícito.
...
begin
  ...
  -- todos os 3 slots foram preenchidos entre a checagem do count e a tentativa (corrida rara)
  return query select false, false, true;
end;
$$ language plpgsql;
